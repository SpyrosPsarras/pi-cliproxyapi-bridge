...
// operators never copy key material into this plugin's configuration.
//
...
func authenticate(cfg pluginConfig, knownKeys []string, headers http.Header) (authenticatedClient, bool) {
	...
	// The key must be one the proxy itself accepts, so an arbitrary string can
	// never pass by matching a stale checkbox.
	if !isKnownKey(token, knownKeys) {
		return authenticatedClient{}, false
	}
	if !cfg.allowAll() && !cfg.keySelected(token) {
		return authenticatedClient{}, false
	}
